...
def _plot(rows, base):
    try:
        ...
        return
    # x-axis: the calibration (cert-fold) unknown fraction = rho * p_deploy
    x = [r["cert_unknown_frac"] for r in rows]
    cov = [r["coverage"] for r in rows]
    fig, ax = plt.subplots(figsize=(6, 4))
    ax.plot(x, cov, marker="o", color="C0", label="empirical coverage")
    ax.axhline(1 - DELTA, ls="--", color="gray", lw=1, label=f"1-delta={1-DELTA:.2f}")
    ax.axvline(P_DEPLOY, ls=":", color="C3", lw=1, label=f"deployment rate p={P_DEPLOY:.2f}")
    ax.set_xlabel("calibration unknown fraction")
    ax.set_ylabel("coverage  P(realized risk <= cert_ucb)")
    ax.set_title("Audit representativeness (A4) stress test")
    ax.set_ylim(0, 1.02)
    ax.legend()
    fig.tight_layout()
