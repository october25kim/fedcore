...
CAMP = os.path.join(REPO, "results", "medmnist_resnext_fedprox_sweep")
MATRIX = os.path.join(CAMP, "final_training_matrix.csv")
MATRIX_SHA256 = "c32de6e83abf181fc3d9f53047192dc656cd3d8cd55ad890268d345b0ea67c6f"
LIVE = os.path.join(CAMP, "live")
RUNS_DEFAULT = os.path.join(REPO, "runs", "medmnist_resnext_fedprox_sweep", "cells")
DATA_NPZ = os.path.join(REPO, "data", "medmnist", "pathmnist.npz")
DATA_NPZ_TISS = os.path.join(REPO, "data", "medmnist", "tissuemnist.npz")

IMAGE = os.environ.get("MEDMNIST_IMAGE", "fedcore-c400r:latest")
C_REPO = "/workspace"
...
def guard_cell(cell):
    ...
    for _npz in (DATA_NPZ, DATA_NPZ_TISS):
        if not os.path.isfile(_npz):
            return False, f"npz_missing:{os.path.basename(_npz)}"
    return True, "ok"
...
def build_inner_cmd(cell, paths, resume):
    ...
    cmd = [
        ...
        "--fedprox-mu", str(cell.get("fedprox_mu", 0.0)),
...
def build_docker_cmd(cell, paths, gpu_uuid, resume):
    cname = "medrx_" + cell["semantic_id"].replace(".", "_").replace("/", "_")[:100]
...
class LiveState:
    ...
    def _flush_locked(self):
        ...
        _atomic_write(os.path.join(LIVE, "status.json"), json.dumps(
            {"campaign": "medmnist_sweep", "dispatcher_pid": self.pid, "dispatcher_sid": self.sid,
...
def kill_orphan_containers():
    try:
        out = subprocess.run(["docker", "ps", "-q", "--filter", "name=medrx_"],
...
def cmd_print_plan(args):
    ...
    print(f"[print-plan] image {IMAGE} present: {IMAGE in imgs} | npz(path+tiss): {os.path.isfile(DATA_NPZ) and os.path.isfile(DATA_NPZ_TISS)}")
    ...
    ok = got == MATRIX_SHA256 and IMAGE in imgs and os.path.isfile(DATA_NPZ) and os.path.isfile(DATA_NPZ_TISS) and nfail == 0
...
def cmd_run(args):
    ...
    try:
        ...
        print(f"[dispatcher] medmnist_sweep cells={len(cells)} gpus={len(gpu_uuids)}")
