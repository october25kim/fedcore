...
CAMP = os.path.join(REPO, "results", "medmnist_sweep")
MATRIX = os.path.join(CAMP, "final_training_matrix.csv")
MATRIX_SHA256 = "5d6562addd3ccf1c988e336dd24c6105fba20748ac6e1e682ae2f6b5bf40251b"
LIVE = os.path.join(CAMP, "live")
RUNS_DEFAULT = os.path.join(REPO, "runs", "medmnist_sweep", "cells")
...
def guard_cell(cell):
    ...
    if not os.path.isfile(DATA_NPZ):
        return False, "pathmnist_npz_missing"
    return True, "ok"
...
def build_docker_cmd(cell, paths, gpu_uuid, resume):
    cname = "medm_" + cell["semantic_id"].replace(".", "_").replace("/", "_")[:100]
...
def kill_orphan_containers():
    try:
        out = subprocess.run(["docker", "ps", "-q", "--filter", "name=medm_"],
...
def cmd_print_plan(args):
    ...
    print(f"[print-plan] image {IMAGE} present: {IMAGE in imgs} | pathmnist.npz: {os.path.isfile(DATA_NPZ)}")
