...
CAMP = os.path.join(REPO, "results", "cifar_backbone_sweep")
MATRIX = os.path.join(CAMP, "final_training_matrix.csv")
MATRIX_SHA256 = "4bdaf63a857283f1c1d5347ee325937613117502c6cfd0f0aa531380a9c45680"
LIVE = os.path.join(CAMP, "live")
RUNS_DEFAULT = os.path.join(REPO, "runs", "cifar_backbone_sweep", "cells")
...
CANARY_IDS = [
    "wrn28_10_plain_fedavg__cifar10__split00__seed0__d0.1",
    "wrn28_10_plain_fedavg__cifar100__split00__seed0__d0.1",
    "resnext29_8x64d_plain_fedavg__cifar10__split00__seed0__d0.1",
    "resnext29_8x64d_plain_fedavg__cifar100__split00__seed0__d0.1",
]
...
def build_docker_cmd(cell, paths, gpu_uuid, resume):
    cname = "cifsw_" + cell["semantic_id"].replace(".", "_").replace("/", "_")[:100]
...
def kill_orphan_containers():
    try:
        out = subprocess.run(["docker", "ps", "-q", "--filter", "name=cifsw_"],
...
def cmd_run(args):
    ...
    rows = _load_matrix()
    fresh = fresh_cells(rows)
    arms_filter = [a for a in (args.arms or "").split(",") if a]
    if arms_filter:
        fresh = [c for c in fresh if c["arm"] in set(arms_filter)]
        print(f"[dispatcher] --arms {arms_filter}: {len(fresh)} fresh cells after filter")
    ...
    # only require canaries whose arm is in the run (the frozen gate already passed for
    # every arm at first launch; a filtered relaunch is a resumption, so included-arm
    # canaries are already complete and re-pass instantly). Default (no --arms) keeps
    # all four canaries, byte-identical to the original behavior.
    canary_ids = [cid for cid in CANARY_IDS if cid in by_id]
    if not canary_ids:
        print(f"FAIL_CLOSED: no canary present for arms={arms_filter or 'ALL'}")
        return 2
...
def main(argv=None):
    ...
    rr.add_argument("--arms", default="",
                    help="comma list of arms to run (e.g. wrn28_10_plain_fedavg); empty = "
                         "all fresh arms. Used by the priority scheduler to run Arm B "
                         "before the deprioritized ResNeXt Arm C.")
    rr.add_argument("--n-splits", dest="n_splits", type=int, default=0,
                    help="run only split indices 0..N-1 (0 = all 10). Simplification: "
                         "5 keeps splits 00-04 (150 cells/arm at 5x5x3x2).")
