...
by this file.  Outputs land only under ``runs/fedisic_convnext_intervention`` (rw
mount) and ``results/fedisic_convnext_intervention/live`` (host).
"""
...
CAMP = os.path.join(REPO, "results", "fedisic_convnext_intervention")
...
INTERVENTION_RUNS = os.path.join(REPO, "runs", "fedisic_convnext_intervention")
...
# FROZEN recipe (design_declaration recipe block; do NOT change).
ROUNDS = 30
WARMUP_ROUNDS = 2
LOCAL_EPOCHS = 1
BATCH_SIZE = 32
...
def write_marker(cell, paths, gpu_uuid, rc, attempt, source):
    ...
    marker = {
        ...
        "campaign": "fedisic_convnext_intervention",
        ...
        "marker_written_by": f"fedisic_convnext_launch.{source}",
...
def build_inner_cmd(cell, paths, resume):
    ...
    cmd = [
        ...
        "--backbone", "convnext_tiny", "--transform", "convnext-imagenet",
        "--optimizer", "adamw", "--loss", "ce", "--pretrained",
...
def build_docker_cmd(cell, paths, gpu_uuid, resume):
    cname = "fic_" + cell["semantic_id"].replace(".", "_").replace("/", "_")[:100]
    ...
    docker = [
        ...
        "-v", f"{REPO}:{C_REPO}:ro",
        "-v", f"{INTERVENTION_RUNS}:{C_REPO}/runs/fedisic_convnext_intervention:rw",
...
class LiveState:
    ...
    def _flush_locked(self):
        ...
        summary = {
            "campaign": "fedisic_convnext_intervention", "mode": self.mode,
...
def kill_orphan_containers():
    try:
        out = subprocess.run(["docker", "ps", "-q", "--filter", "name=fic_"],
