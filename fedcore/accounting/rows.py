...
def _budget(
    ...
    ``conditional_risk_certificate`` splits ``delta`` internally: ``delta/J`` over J
    rbar bounds (simplex), or ``delta/(3J)`` over 3J bounds -- rbar, alow, ahigh --
    (box). Either way the risk certificate spends exactly ``delta``.

    ``certify._coverage_lcb`` then spends a SECOND, independent budget: J alow bounds
    at ``delta/(2J)``, i.e. ``delta/2``. The deployment decision (``U <= alpha``)
    rests on the risk certificate alone and is valid at ``delta``. The headline pair
    "risk <= alpha AND coverage >= L" is a conjunction of both bounds and, by a union
    bound, holds at ``1 - 1.5*delta``. Logged, not silently absorbed.
    """
    J = max(int(n_strata), 1)
    if "box" in variant:
        eps_risk, n_risk = delta / (3.0 * J), 3 * J
    else:
        eps_risk, n_risk = delta / J, J
    eps_cov, n_cov = delta / (2.0 * J), J
    spent_risk = eps_risk * n_risk
    spent_cov = eps_cov * n_cov
    return {
        "delta_total_declared": delta,
        "eps_per_stratum_risk": eps_risk,
        "n_cp_bounds_risk": n_risk,
        "delta_spent_risk_certificate": spent_risk,
        "eps_per_stratum_coverage": eps_cov,
        "n_cp_bounds_coverage": n_cov,
        "delta_spent_coverage_lcb": spent_cov,
        "delta_spent_joint_headline_claim": spent_risk + spent_cov,
    }
